// Validation Pipeline Sourate 95
...
async function validate() {
  console.log('=== VALIDATION SOURATE 95 ===\n');
  ...
  const { data: va } = await sb.from('verse_analyses').select('verse_id, segments, translation_explanation, full_translation, translation_arab, segments_step1, verses!inner(verse_num)')
    .eq('verses.surah_id', 95).order('verses(verse_num)');

  if (!va || va.length === 0) { err('Aucune verse_analyses pour sourate 95'); return; }
  ...
  // v2 has no VWA (proper nouns)
  const expectedVWA = { 1: 2, 2: 0, 3: 2, 4: 4, 5: 2, 6: 6, 7: 2, 8: 3 };
  for (let vn = 1; vn <= 8; vn++) {
    const words = byVerse[vn] || [];
    if (vn === 2) {
      if (words.length === 0) ok(`v${vn}: 0 VWA (noms propres — attendu)`);
      else warn(`v${vn}: ${words.length} VWA trouvés (attendu: 0 noms propres)`);
      continue;
    }
  ...
  console.log('\n=== TRADUCTIONS COMPLÈTES SOURATE 95 ===');
  for (const v of va) {
    const vn = v.verses.verse_num;
    if (v.segments) {
      const segs = typeof v.segments === 'string' ? JSON.parse(v.segments) : v.segments;
