...
async function validate() {
  ...
  // 1. Get all verse_analyses for S107 (verse_ids 6198-6204)
  const s107VerseIds = [6198,6199,6200,6201,6202,6203,6204];
  const { data: verses } = await db
    .from('verse_analyses')
    .select('*')
    .in('verse_id', s107VerseIds)
    .order('verse_id');

  // Add verse_number for display
  for (const v of verses) v.verse_number = v.verse_id - 6197;

  console.log(`=== VALIDATION SOURATE 107 ===\n`);
  console.log(`Versets trouvés: ${verses.length}`);

  // 2. Get all VWA
  const verseIds = s107VerseIds;
  ...
  // 3. Get all word_meanings for all roots used
  ...
  for (const v of verses) {
    ...
    for (const seg of segs) {
      ...
      if (!sr) { errors.push(`V${v.verse_number} [${wk}]: sense_retenu manquant`); continue; }

      const concepts = wmByKey[wk] || {};
      const allSenses = Object.values(concepts).flat();
      if (!allSenses.includes(sr)) {
        errors.push(`V${v.verse_number} [${wk}]: sense_retenu "${sr}" n'existe PAS dans word_meanings. Sens disponibles: ${allSenses.join(', ')}`);
      }
    }
  }
  ...
  for (const vwa of vwas) {
    ...
    for (const axe of axes) {
      ...
      if (sentences.length < 4) {
        warnings.push(`VWA ${vwa.id} [${vwa.word_key}]: ${axe} n'a que ${sentences.length} phrases (min: 5)`);
      }
    }

    // Check all word_meanings concepts are represented
    for (const wmConcept of Object.keys(wmConcepts)) {
      if (!ax.concepts[wmConcept]) {
        warnings.push(`VWA ${vwa.id} [${vwa.word_key}]: concept "${wmConcept}" de word_meanings absent de l'analyse`);
      }
    }
    ...
    // Check no Arabic characters in axes (allow phonetic like ara'ayta)
    for (const axe of axes) {
  ...
  // CHECK 6: sense_chosen in VWA matches word_meanings concept sense
  ...
  if (errors.length === 0) {
    console.log(`\n✅ VALIDATION RÉUSSIE — Sourate 107 conforme aux règles`);
  } else {
  ...
  console.log(`\n========== RÉSUMÉ PAR VERSET ==========`);
  for (const v of verses) {
    const vVwas = vwas.filter(w => w.verse_id === v.id);
    console.log(`\nVERSET 107:${v.verse_number} — ${vVwas.length} mots analysés`);
