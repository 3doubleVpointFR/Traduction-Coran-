// Validation Pipeline Sourate 91
...
async function validate() {
  console.log('=== VALIDATION SOURATE 91 ===\n');

  // 1. Check all 15 verses have verse_analyses with segments
  ...
  const { data: va } = await sb.from('verse_analyses').select('verse_id, segments, translation_explanation, full_translation, verses!inner(verse_num)')
    .eq('verses.surah_id', 91).order('verses(verse_num)');

  if (!va || va.length === 0) { err('Aucune verse_analyses pour sourate 91'); return; }
  if (va.length === 15) ok(`${va.length} verse_analyses trouvées (15/15)`);
  else err(`${va.length} verse_analyses trouvées (attendu: 15)`);
  ...
  const rootIds = {
    shms: 1117, dhuh: 2138, qmr: 1783, tlw: 730, nhr: 397, jly: 1974,
    lyl: 528, ghw: 306, smw: 249, bny: 386, ard: 324, Thw: 2622,
    nfs: 298, swy: 295, lhm: 313, fjr: 587, wqy: 277, flh: 280,
    zkw: 504, xyb: 2623, dss: 2340, kdb: 474, thmd: 1892, tghw: 1112,
    beth: 545, shqy: 2176, qwl: 307, rsl: 688, nqa: 1893, sqy: 584,
    eqr: 1261, dmdm: 2624, rbb: 253, dnb: 1229, xwf: 759, eqb: 814
  };
  ...
  console.log('\n=== TRADUCTIONS COMPLÈTES SOURATE 91 ===');
  for (const v of va) {
    const vn = v.verses.verse_num;
    if (v.segments) {
      const segs = typeof v.segments === 'string' ? JSON.parse(v.segments) : v.segments;
