// Validation Pipeline Sourate 92
...
async function validate() {
  console.log('=== VALIDATION SOURATE 92 ===\n');

  // 1. Check all 21 verses have verse_analyses with segments
  console.log('--- 1. Vérification verse_analyses ---');
  const { data: va } = await sb.from('verse_analyses').select('verse_id, segments, translation_explanation, full_translation, verses!inner(verse_num)')
    .eq('verses.surah_id', 92).order('verses(verse_num)');

  if (!va || va.length === 0) { err('Aucune verse_analyses pour sourate 92'); return; }
  if (va.length === 21) ok(`${va.length} verse_analyses trouvées (21/21)`);
  else err(`${va.length} verse_analyses trouvées (attendu: 21)`);
  ...
  const rootIds = {
    lyl: 528, ghw: 306, nhr: 397, jly: 1974, xlq: 344, dkr: 1688, anth: 960,
    sey: 757, 'štt': 2613, ety: 2040, wqy: 277, sdq: 384, hsn: 574, ysr: 968,
    bxl: 2068, gny: 2584, kdb: 474, esr: 969, mwl: 1148, rdya: 2620,
    hdy: 261, axr: 292, awl: 337, ndhr: 802, nar: 1422, lzw: 2621,
    sly: 1469, shqy: 2176, wly: 599, jnb: 1412, zkw: 504, nem: 264,
    jzy: 531, bgh: 683, wjh: 747, rbb: 253, ely: 265, rdw: 812
  };
  ...
  for (const v of va) {
    ...
    if (/\bpipeline\b/i.test(v.translation_explanation)) warn(`v${vn}: jargon "pipeline" dans translation_explanation`);
  }
  ...
  console.log('\n=== TRADUCTIONS COMPLÈTES SOURATE 92 ===');
  for (const v of va) {
    const vn = v.verses.verse_num;
    if (v.segments) {
      const segs = typeof v.segments === 'string' ? JSON.parse(v.segments) : v.segments;
