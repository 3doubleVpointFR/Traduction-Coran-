...
async function validate() {
  ...
  console.log(`=== VALIDATION SOURATE 2:21-30 ===\n`);
  ...
  // CHECK 2: Daily phrases (at least 3 per root)
  ...
  // CHECK 9: sense_chosen in VWA
  console.log(`--- CHECK 9: sense_chosen dans VWA ---`);
  for (const vwa of vwas) {
    const ax = vwa.analysis_axes;
    if (!ax || !ax.sense_chosen) {
      errors.push(`VWA ${vwa.id} [${vwa.word_key}]: sense_chosen manquant`);
    }
  }
  console.log(`  Vérifié\n`);
  ...
  if (errors.length === 0) {
    console.log(`\n✅ VALIDATION RÉUSSIE — Sourate 2:21-30 conforme aux règles`);
  } else {
