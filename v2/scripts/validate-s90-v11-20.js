// Validation Pipeline Sourate 90 (v11-20)
...
async function validate() {
  console.log('=== VALIDATION SOURATE 90 (v11-20) ===\n');
  ...
  const { data: va } = await sb.from('verse_analyses').select('verse_id, segments, translation_explanation, translation_arab, full_translation, verses!inner(verse_num, surah_id)')
    .eq('verses.surah_id', 90).gte('verses.verse_num', 11).lte('verses.verse_num', 20).order('verses(verse_num)');

  if (!va || va.length === 0) { err('Aucune verse_analyses pour sourate 90 v11-20'); return; }
  ...
  const rootIds = {
    qHm: 2631, eqb: 814, dry: 2098, fkk: 2615, rqb: 955,
    tem: 577, ywm: 257, sghb: 2632, ytm: 664, qrb: 467,
    skn: 462, trb: 1156, amn: 287, wsy: 813, sbr: 502,
    rhm: 251, shb: 476, ymn: 1028, kfr: 294, ayt: 475,
    shAm: 2633, nar: 1422, wsd: 2422
  };
  ...
  console.log('\n=== TRADUCTIONS COMPLÈTES SOURATE 90 (v11-20) ===');
  for (const v of va) {
    const vn = v.verses.verse_num;
    console.log(`v${vn}: ${v.translation_arab || '(pas de traduction)'}`);
  }
}
