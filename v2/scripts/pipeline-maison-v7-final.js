...
async function run() {
  ...
  // ── PURGE ──
  log('\u25B8 PURGE COMPL\u00C8TE...')
  await db.from('verse_analyses').delete().eq('verse_id', VERSE_ID)
  await db.from('verse_word_analyses').delete().eq('verse_id', VERSE_ID)
  for (const [key, root] of Object.entries(ROOTS)) {
    await db.from('word_meanings').delete().eq('analysis_id', root.id)
    await db.from('word_daily').delete().eq('analysis_id', root.id)
  }
  log('  \u2705 Purg\u00E9 : verse_analyses, verse_word_analyses, word_meanings, word_daily')
  log('')

  // ── ÉTAPE 1 — Appel LLM ──
  log('\u2550\u2550\u2550 ÉTAPE 1 \u2014 TAG VERSE (GPT-4.1-mini) \u2550\u2550\u2550')
  log('  Appel LLM en cours...')
  const step1Resp = await fetch(`${API_BASE}/api/analyze/step1-only`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ surah_id: SURAH_ID, verse_num: VERSE_NUM })
  })
  if (!step1Resp.ok) { log('  \u274C ERREUR étape 1: ' + step1Resp.status); process.exit(1) }
  const step1Data = await step1Resp.json()
  const segments = step1Data.segments
  log(`  \u2705 ${segments.length} segments identifiés`)
  const importants = segments.filter(s => s.type === 'important')
  const particles = segments.filter(s => s.type === 'particle')
  const uniqueRoots = [...new Set(importants.map(s => s.key))]
  log(`  ${importants.length} mots importants, ${particles.length} particules, ${uniqueRoots.length} racines uniques`)
  for (const s of segments) {
    if (s.type === 'particle') {
      log(`  ${s.position}. [PARTICULE] ${s.arabic} (${s.phon}) \u2192 "${s.fr}"`)
    } else {
      const details = [s.pos, s.definite ? 'DÉFINI' : null, s.voice === 'passif' ? 'voix passive' : null].filter(Boolean).join(', ')
      log(`  ${s.position}. [IMPORTANT] ${s.arabic} (${s.key}) \u2014 ${details}`)
    }
  }
  log('')
