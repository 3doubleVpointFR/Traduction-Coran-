...
(async () => {
  console.log('=== VALIDATION S87 v1-10 ===\n');

  for (let vnum = 1; vnum <= 10; vnum++) {
    const vid = 5948 + vnum;
    console.log(`\n--- v${vnum} (${vid}) ---`);
    ...
    for (const vwa of vwas) {
      ...
      // Check concept_chosen exists in word_meanings DB
      const { data: wm } = await sb.from('word_meanings').select('concept').eq('analysis_id', getAnalysisId(vwa.word_key));
      if (wm) {
        const dbConcepts = [...new Set(wm.map(m => m.concept))];
        if (!dbConcepts.includes(axes.concept_chosen)) {
          err('VWA ' + vwa.word_key + ': concept_chosen "' + axes.concept_chosen + '" absent de word_meanings DB. Concepts DB: ' + dbConcepts.join(', '));
        }
      }
      ...
      // Check sense_chosen matches segment sense_retenu
      if (va.segments) {
        const matchingSegs = va.segments.filter(s => s.word_key === vwa.word_key);
        for (const seg of matchingSegs) {
          // Check sense exists in word_meanings
          const { data: wmSenses } = await sb.from('word_meanings').select('sense').eq('analysis_id', getAnalysisId(vwa.word_key));
          if (wmSenses) {
            const dbSenses = wmSenses.map(m => m.sense);
            if (!dbSenses.includes(seg.sense_retenu)) {
              warn('VWA ' + vwa.word_key + ': sense_retenu "' + seg.sense_retenu + '" absent de word_meanings. Sens DB: ' + dbSenses.slice(0, 5).join(', '));
            }
          }
        }
      }
...
function getAnalysisId(word_key) {
  const map = {
    'sbh': 440, 'smw': 249, 'rbb': 253, 'elw': 371,
    'xlq': 344, 'swy': 295, 'qdr': 373, 'hdy': 261,
    'xrj': 388, 'rey': 1618, 'jel': 359, 'ghw2': 2646, 'Hw': 2647,
    'qra': 512, 'nsy': 511, 'shya': 423, 'elm': 254,
    'jhr': 553, 'xfy': 1174, 'ysr': 968,
    'ðkr': 1051, 'nfe': 782, 'kšy': 1610
  };
  return map[word_key] || 0;
}
