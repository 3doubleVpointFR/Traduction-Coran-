// Validation Pipeline Sourate 89 (v1-10)
...
async function validate() {
  console.log('=== VALIDATION SOURATE 89 (v1-10) ===\n');
  ...
  const { data: va } = await sb.from('verse_analyses').select('verse_id, segments, translation_explanation, translation_arab, full_translation, verses!inner(verse_num, surah_id)')
    .eq('verses.surah_id', 89).gte('verses.verse_num', 1).lte('verses.verse_num', 10).order('verses(verse_num)');

  if (!va || va.length === 0) { err('Aucune verse_analyses pour sourate 89 v1-10'); return; }
  ...
  for (const w of vwa || []) {
    ...
    for (const [cname, cdata] of Object.entries(axes.concepts)) {
      if (!cdata.status) err(`v${vn} ${w.word_key}: "${cname}" sans status`);
      if (!cdata.senses || cdata.senses.length === 0) err(`v${vn} ${w.word_key}: "${cname}" sans senses`);
      if (!cdata.proof_ctx) err(`v${vn} ${w.word_key}: "${cname}" sans proof_ctx`);

      if (cdata.status === 'retenu') {
        hasRetenu = true;
        for (const axeName of ['axe1_verset', 'axe2_voisins', 'axe3_sourate', 'axe4_coherence', 'axe5_frequence']) {
          if (!cdata[axeName]) err(`v${vn} ${w.word_key}: retenu "${cname}" sans ${axeName}`);
      ...
      if (cdata.status === 'probable' || cdata.status === 'peu_probable') {
        for (const axeName of ['axe1_verset', 'axe2_voisins', 'axe3_sourate', 'axe4_coherence', 'axe5_frequence']) {
          if (!cdata[axeName]) err(`v${vn} ${w.word_key}: ${cdata.status} "${cname}" sans ${axeName}`);
      ...
      for (const key of ['axe1_verset', 'axe2_voisins', 'axe3_sourate', 'axe4_coherence', 'axe5_frequence', 'proof_ctx']) {
        if (cdata[key] && /[\u0600-\u06FF]/.test(cdata[key])) {
          err(`v${vn} ${w.word_key}: arabe dans ${key} de "${cname}"`);
        }
      }
      ...
      for (const key of ['axe1_verset', 'axe2_voisins', 'axe3_sourate', 'axe4_coherence', 'axe5_frequence']) {
        if (cdata[key] && /\b(the|and|of|that|which|with|from|this|but|not|are|was|for|have|has|been|its|his|her)\b/i.test(cdata[key])) {
          warn(`v${vn} ${w.word_key}: mots anglais possibles dans ${key} de "${cname}"`);
        }
      }
    }

    if (!hasRetenu) err(`v${vn} ${w.word_key}: aucun retenu`);
  ...
  for (const v of va) {
    ...
    for (const seg of segs) {
      ...
      if (!waData || waData.length === 0) { err(`v${vn} seg "${seg.fr}": word_key "${seg.word_key}" n'existe pas`); continue; }
  ...
  // 7. Check daily phrases
  console.log('\n--- 7. Vérification phrases quotidiennes ---');
  const rootIds = {
    fjr: 587, lyl: 528, eshr: 589, shfe: 533, wtr: 2634,
    sry: 523, qsm: 1433, hjr: 376, ray: 552, fel: 374,
    rbb: 253, emd: 1541, xlq: 344, mvl: 2599, bld: 793,
    jwb: 963, Sxr: 2635, wdy_v: 2637, wtd: 2636
  };
  ...
  // Print translations
  console.log('\n=== TRADUCTIONS COMPLÈTES SOURATE 89 (v1-10) ===');
  for (const v of va) {
