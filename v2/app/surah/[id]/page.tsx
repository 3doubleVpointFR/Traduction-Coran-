...
export default async function SurahPage({ params, searchParams }: Props) {
  ...
  if (pageVerseIds.length > 0) {
    const [wordsRes, analysesRes] = await Promise.all([
      ...
      db
        .from('verse_analyses')
        .select('id, verse_id, segments, full_translation, translation_arab, translation_explanation, summary_short, summary_long, surah_conclusion, model_used, prompt_version, generated_at')
        .in('verse_id', pageVerseIds)
        .not('translation_arab', 'is', null)
