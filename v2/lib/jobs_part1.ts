  ...
  try {
    ...
    for (let wi = 0; wi < importantWords.length; wi++) {
      ...
      if (existing && existing.analysis_step) {
        console.log(`│  ✓ Étymologie réutilisée depuis DB pour ${word.key} (${word.pos})`)
      } else {
        ...
        const { count: existingDailyP1 } = await db.from('word_daily').select('*', { count: 'exact', head: true }).eq('analysis_id', analysisId)
        if ((existingDailyP1 ?? 0) > 0) {
          console.log(`    [SKIP] ${existingDailyP1} daily phrases already exist for analysis_id=${analysisId}`)
